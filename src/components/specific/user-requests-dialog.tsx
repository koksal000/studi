...
import { Card, CardContent, CardHeader, CardTitle as CardTitlePrimitive, CardDescription as CardDesc, CardFooter } from '@/components/ui/card'; // Renamed CardTitle to avoid conflict
import { useContactMessages, type ContactMessage } from '@/hooks/use-contact-messages';
import { Button } from '@/components/ui/button';
import { Loader2, Mail, MessageSquare, User, CalendarDays, Inbox, Send } from 'lucide-react';
import { SendUserMessageDialog } from './send-user-message-dialog';
...
export function UserRequestsDialog({ isOpen, onOpenChange }: UserRequestsDialogProps) {
  ...
  const [userToMessage, setUserToMessage] = useState<Partial<UserProfile> & { email: string } | null>(null);
  ...
  const handleReplyClick = (message: ContactMessage) => {
    if (message.email) {
      setUserToMessage({ 
        id: message.email, // Using email as a proxy for ID here for the dialog
        name: message.name,
        surname: '', // Surname is not available in contact message
        email: message.email 
      });
                    ...
                    <CardContent>
                     ...
                     {msg.email && (
                        <CardFooter className="pt-2">
                           <Button variant="outline" size="sm" onClick={() => handleReplyClick(msg)}>

...
export const PeerProvider = ({ children }: PeerProviderProps) => {
  ...
  const connectToPeer = useCallback((targetPeerId: string, isInitialContactAttempt = false): DataConnection | undefined => {
    ...
    if (targetPeerId === peerId) { // Use the state peerId for self-check
    ...
    conn.on('error', (err) => {
      ...
      if (isInitialContactAttempt && targetPeerId === DEFAULT_CONTACT_PEER_ID) {
        toast({
          ...
          description: `Varsayılan kontak noktasına (${targetPeerId.substring(0,12)}...) bağlanılamadı. Bu peer çevrimiçi değilse, ağ sorunları varsa veya bu ID'yi kullanan bir cihaz yoksa, güncel duyurular alınamayabilir. Hata: ${err.message}`,
          variant: 'destructive',
          duration: 10000
        });
      } else if (targetPeerId !== peerId) { // Avoid self-connection error toasts
    ...
    return conn;
  }, [peerId, toast, dataHandler]); // Added peerId to dependencies
  ...
  const initializePeer = useCallback(() => {
    ...
    let peerIdToAttempt: string | undefined = localStorage.getItem(MY_PEER_ID_KEY);

    if (!peerIdToAttempt) {
        console.log(`InitializePeer: No stored ID. Attempting to initialize as DEFAULT_CONTACT_PEER_ID: ${DEFAULT_CONTACT_PEER_ID}`);
        peerIdToAttempt = DEFAULT_CONTACT_PEER_ID;
    } else {
        console.log(`InitializePeer: Attempting to reuse stored ID: ${peerIdToAttempt}`);
    }

    if(peerInstanceRef.current && (peerInstanceRef.current.destroyed || (peerIdToAttempt && peerInstanceRef.current.id !== peerIdToAttempt))){
    ...
    console.log(`InitializePeer: Creating new Peer instance, attempting ID: ${peerIdToAttempt}`);
    const newPeer = new PeerModule(peerIdToAttempt);
    ...
    newPeer.on('open', (currentId) => {
      ...
      if (peerIdToAttempt === DEFAULT_CONTACT_PEER_ID && currentId !== DEFAULT_CONTACT_PEER_ID) {
        console.warn(`InitializePeer: Tried to become DEFAULT_CONTACT_PEER_ID ('${DEFAULT_CONTACT_PEER_ID}'), but it was taken. Opened with random ID: '${currentId}'. This new ID will be saved to localStorage.`);
        toast({ title: 'Kontak Peer ID Alınamadı', description: `Varsayılan kontak ID ('${DEFAULT_CONTACT_PEER_ID.substring(0,12)}') başkası tarafından kullanılıyor. Rastgele bir ID ('${currentId.substring(0,12)}') ile devam ediliyor.`, variant: 'destructive', duration: 8000});
      } else if (peerIdToAttempt === DEFAULT_CONTACT_PEER_ID && currentId === DEFAULT_CONTACT_PEER_ID) {
        toast({ title: 'Kontak Peer Aktif', description: `Bu cihaz varsayılan kontak noktası oldu: ${currentId.substring(0,12)}...` });
      } else {
        toast({ title: 'P2P Ağı Aktif', description: `Kimliğiniz: ${currentId.substring(0,12)}...` });
      }

      localStorage.setItem(MY_PEER_ID_KEY, currentId); // Always save the ID that was actually opened
      ...
      if (currentId !== DEFAULT_CONTACT_PEER_ID) {
          ...
          console.log(`This peer IS the default contact peer: ${currentId}. Not attempting to connect to self.`);
      }
    ...
    newPeer.on('error', (err) => {
      ...
      if (err.type === 'unavailable-id') {
        ...
        if (peerIdToAttempt === DEFAULT_CONTACT_PEER_ID) {
            console.warn(`DEFAULT_CONTACT_PEER_ID ('${DEFAULT_CONTACT_PEER_ID}') is unavailable. This instance will get a random ID.`);
            // Toast for this specific case is handled in 'open' event if currentId !== peerIdToAttempt
        } else if (peerIdToAttempt) { // Tried to use a stored ID and it was unavailable
            console.warn(`Stored peer ID ${peerIdToAttempt} is unavailable. Removing it.`);
            localStorage.removeItem(MY_PEER_ID_KEY);
            toast({ title: 'P2P Kimlik Sorunu', description: `Saklanan P2P kimliğiniz (${peerIdToAttempt.substring(0,12)}...) kullanılamadı. Yeni bir kimlik denenecek.`, variant: 'destructive', duration: 8000});
        } else { // Should not happen if peerIdToAttempt is always set
        ...
        peerInstanceRef.current = null;
        setPeerId(null); // Reset peerId so useEffect can try re-init
        isInitializingRef.current = false; // Allow re-initialization
        // No return here, want it to fall through and potentially re-initialize via useEffect
         ...
         // This case is handled by connectToPeer's error handler with a specific toast
      ...
      // For errors other than 'unavailable-id', if peer exists and is not destroyed, destroy it.
      // 'unavailable-id' already handles destruction and reset.
      if (err.type !== 'unavailable-id' && peerInstanceRef.current && !peerInstanceRef.current.destroyed) {
          console.warn("Destroying peer instance due to error:", err.type);
          peerInstanceRef.current.destroy();
          peerInstanceRef.current = null;
          setPeerId(null);
      }
      // For most errors, reset initializing flag so it can try again if conditions allow
      if (err.type !== 'unavailable-id') {
        isInitializingRef.current = false;
      }
    });
    ...
    newPeer.on('disconnected', () => {
        ...
        // Toast for this event has been removed to reduce noise as PeerJS handles reconnection.
    });
  ...
  }, [PeerModule, user, toast, connectToPeer, dataHandler, endMediaCallInternals]);
  ...
  useEffect(() => {
    if (PeerModule && user && !peerId && !isInitializingRef.current) {
      console.log(`Peer init useEffect: Conditions met. Initializing peer. Peer Lib: ${!!PeerModule}, User: ${!!user}, PeerInstanceRef: ${!!peerInstanceRef.current}, PeerId: ${peerId}, Initializing: ${isInitializingRef.current}`);
      initializePeer();
    }
  ...
  }, [PeerModule, user, peerId, initializePeer, endMediaCallInternals]); // Added endMediaCallInternals to dependencies

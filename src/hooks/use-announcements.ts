...
export function useAnnouncements() {
  ...
  useEffect(() => {
    ...
    const handleData = (data: any, fromPeerId: string) => {
      ...
      if (data.type === 'NEW_ANNOUNCEMENT') {
        ...
        setAnnouncements(prev => {
         ...
         if (peerContext.peerId) { // Check if peerId is available
    ...
    // Note: No explicit unregister needed if PeerContext handles teardown or dataHandler.current is overwritten.
    // If useAnnouncements unmounts while PeerContext persists, the registered handleData might hold stale closures
    // for isAdmin, user, siteNotificationsPreference if not properly managed with useCallback or if their values change
    // without re-triggering this useEffect. The current dependencies aim to mitigate this.

  }, [peerContext, isAdmin, user, siteNotificationsPreference, setAnnouncements]); // Dependencies for the useEffect
  ...
  const addAnnouncement = useCallback((newAnnouncementData: Omit<Announcement, 'id' | 'date' | 'author' | 'authorId'>) => {
    ...
    const announcement: Announcement = {
      ...
      authorId: peerContext.peerId || undefined
    };

...
export function useGallery() {
  ...
  useEffect(() => {
    ...
    es.onopen = () => {
      ...
      // Initial data will be sent by the stream upon connection if API fetch is successful there.
    };
    ...
    es.onmessage = (event) => {
      try {
        ...
        if (isLoading) setIsLoading(false); // Set loading false after first message
      } catch (error) {
  ...
  }, [isLoading]); // isLoading added to dependencies
  ...
  const addGalleryImage = useCallback(async (payload: NewGalleryImagePayload) => {
    ...
    // No optimistic UI update here. API call first.
